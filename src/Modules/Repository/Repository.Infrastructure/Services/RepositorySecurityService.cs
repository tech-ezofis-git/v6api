        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var rule in hideRules)
        {
            ...
            foreach (var condition in rule.Conditions)
            {
                ...
                if (op is "contains")
                {
                    parts.Add($"LTRIM(RTRIM(CAST([{column}] AS NVARCHAR(MAX)))) LIKE {paramName}");
                    parameters.Add((paramName, $"%{value}%"));
                }
                else if (op is "notequals" or "ne" or "!=")
                {
                    parts.Add($"(LTRIM(RTRIM(CAST([{column}] AS NVARCHAR(MAX)))) <> {paramName} OR [{column}] IS NULL)");
                    parameters.Add((paramName, value));
                }
                else
                {
                    parts.Add($"LTRIM(RTRIM(CAST([{column}] AS NVARCHAR(MAX)))) = {paramName}");
                    parameters.Add((paramName, value));
                }
            }
