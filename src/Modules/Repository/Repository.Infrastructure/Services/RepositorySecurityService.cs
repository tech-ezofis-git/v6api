        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var rule in hideRules)
        {
            ...
            foreach (var condition in rule.Conditions)
            {
                ...
                var colRef = RepositorySqlHelper.ColumnRef(column);

                if (op is "contains")
                {
                    parts.Add($"BTRIM(CAST({colRef} AS text)) LIKE {paramName}");
                    parameters.Add((paramName, $"%{value}%"));
                }
                else if (op is "notequals" or "ne" or "!=")
                {
                    parts.Add($"(BTRIM(CAST({colRef} AS text)) <> {paramName} OR {colRef} IS NULL)");
                    parameters.Add((paramName, value));
                }
                else
                {
                    parts.Add($"BTRIM(CAST({colRef} AS text)) = {paramName}");
                    parameters.Add((paramName, value));
                }
            }
