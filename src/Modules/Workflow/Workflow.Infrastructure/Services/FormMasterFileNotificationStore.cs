...
/// <summary>v5 dbo.notification table — required by Python master file import and move-next notifications.</summary>
internal static class FormMasterFileNotificationStore
{
    ...
    private static async Task EnsureExtraColumnsAsync(NpgsqlConnection connection, CancellationToken cancellationToken)
    {
        foreach (var (name, definition) in ExtraColumns)
        {
            ...
            var sql = $"""ALTER TABLE dbo.notification ADD COLUMN IF NOT EXISTS "{name}" {definition};""";
        ...
        CancellationToken cancellationToken)
    {
        ...
        cmd.Parameters.AddWithValue("@CreatedAtUtc", atUtc.ToUniversalTime());
